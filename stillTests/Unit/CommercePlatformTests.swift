...
final class CommercePlatformTests: XCTestCase {
  func testBundledStoreKitConfigurationMatchesCatalog() throws {
    let configurationURL = try XCTUnwrap(
      Bundle.main.url(forResource: "StoreKit", withExtension: "storekit")
    )
    ...
    XCTAssertEqual(
      ...
      StillCommerceCatalog.catalog.productIDs
    )
  }
  ...
  func testPreviewClientCoversProductsRestoreAndUpdates() async throws {
    ...
    XCTAssertEqual(Set(products.map(\.id)), StillCommerceCatalog.catalog.productIDs)

    for productID in StillCommerceCatalog.catalog.productIDs {
  ...
  func testCatalogValidatesPremiumProducts() throws {
    let catalog = try SubscriptionCatalog.validating(
  ...
  func testStoreKitClientRejectsUnverifiedValues() {
    let result: VerificationResult<Int> = .unverified(
      42,
      .invalidSignature
    )

    XCTAssertThrowsError(
      try StoreKitSubscriptionClient.verifiedValue(from: result)
    ) { error in
  ...
  func testCatalogRejectsUnknownPremiumProduct() {
    XCTAssertThrowsError(
      try SubscriptionCatalog.validating(
        productIDs: ["monthly"],
